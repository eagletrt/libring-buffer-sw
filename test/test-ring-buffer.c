...
void setUp(void) {
    ...
    ring_buffer_init(&point_buf, Point, 10, NULL, NULL, &arena);
}
...
void check_ring_buffer_init_with_null(void) {
    TEST_ASSERT_EQUAL_INT(RING_BUFFER_NULL_POINTER, ring_buffer_init(NULL, float, 3, NULL, NULL, &arena));
}
void check_ring_buffer_init_return_value(void) {
    RingBuffer(float, 3) buf;
    TEST_ASSERT_EQUAL_INT(RING_BUFFER_OK, ring_buffer_init(&buf, float, 3, NULL, NULL, &arena));
}
